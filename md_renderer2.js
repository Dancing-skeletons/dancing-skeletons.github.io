...
function chordStaticPlugin(md) {
  function tokenizeStaticChord(state, silent) {
    ...
    if (!silent) {
      const token = state.push('html_inline', '', 0);
      token.content = `<span class="chord chord-static" data-chord="${content}">${content}</span>`;
    }
...
// register alongside the existing chordPlugin
md.use(chordPlugin);
...
//  <button id="toggle-chords">Accords</button>
  <div class="control-group">
    <label class="control-checkbox"><input type="checkbox" id="chord-toggle" checked> Accords</label>
    <input type="range" id="chord-size" min="0.4" max="1.6" step="0.05" value="1">
  </div>
...
function applyChordSize(){
  document.querySelectorAll('.chord-diagram').forEach(el => el.setAttribute('size', chordSizeSlider.value));
}
